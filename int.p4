...
control __sink(inout int_headers hdr, in int_metadata meta, inout standard_metadata_t std_meta) {
...
control __transit(inout int_headers hdr, inout int_metadata meta, in standard_metadata_t std_meta) {
        ...
        action int_set_header_1() {
            ...
            hdr.port_ids.egress_port_id  = (bit<16>)std_meta.egress_port;
        ...
        action int_set_header_5() {
            ...
            bit<64> _timestamp = (bit<64>)std_meta.egress_global_timestamp;
...
control int_egress(inout int_headers hdr, inout int_metadata meta, inout standard_metadata_t std_meta) {
   apply {
      ...
      log_msg("===INT egress, shim: {}, int_node: {}", {hdr.shim.int_type, meta.int_node});
      //must process only INT packets
      if (!hdr.shim.isValid())
         return;


      if( (meta.int_node & INT_NODE_TRANSIT) != 0 ){
         __l4s.apply( hdr, meta, std_meta );
         __transit.apply( hdr, meta, std_meta );
      }

      if( (meta.int_node & INT_NODE_SINK) != 0 )
