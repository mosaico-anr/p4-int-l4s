...
function rebuild_attacker(){
	...
	if [[ "$LAST_POWER_ATTACK" == "$POWER_ATTACK" ]]; then
		...
		run-on-host "$SERVER_CTRL_IP" "cd $picoquic_path && (rm -rf build; ./rebuild-picoquic.sh srv l4s unrespECN $POWER_ATTACK; git diff)"
		run-on-host "$CLIENT_CTRL_IP" "cd $picoquic_path && (rm -rf build; ./rebuild-picoquic.sh cli l4s unrespECN $POWER_ATTACK; git diff)"
	fi
}
